export const content = {
  welcomeHero: {
    en: {
      title: "10 Months of Love, Laughter & Beautiful Memories ❤️",
      subtitle: "A digital love letter celebrating our journey together",
      button: "Begin Our Journey"
    },
    ta: {
      title: "10 மாதங்கள் அன்பும், சிரிப்பும், அழகான நினைவுகளும் ❤️",
      subtitle: "நமது காதல் பயணத்தை கொண்டாடும் ஒரு டிஜிட்டல் காதல் கடிதம்",
      button: "நமது பயணத்தை தொடங்கலாம்"
    }
  },
  timeline: {
    en: {
      title: "Our Love Story Timeline",
      subtitle: "A month-by-month walkthrough of our beautiful 10 months together",
      months: [
        {
          title: "Month 1: The Spark",
          date: "August 15, 2025",
          description: "Where it all began. The moment our paths crossed and a beautiful new chapter started in our lives. Every conversation felt like home."
        },
        {
          title: "Month 2: Growing Closer",
          date: "September 15, 2025",
          description: "Late-night calls, endless laughter, and sharing secrets. Realizing how incredibly special and unique our connection is."
        },
        {
          title: "Month 3: Beautiful Dates",
          date: "October 15, 2025",
          description: "Our dates, sweet surprises, and quiet moments. Every second spent with you made me realize I want you in my life forever."
        },
        {
          title: "Month 4: Deep Connection",
          date: "November 15, 2025",
          description: "Facing life's small hurdles together and finding an anchor in each other. Your presence became my ultimate comfort zone."
        },
        {
          title: "Month 5: Halfway to Forever",
          date: "December 15, 2025",
          description: "Five months of beautiful dates and sweet talks. My love for you only grows deeper, stronger, and more resilient every day."
        },
        {
          title: "Month 6: Half a Year of Happiness",
          date: "January 15, 2026",
          description: "Six months of pure joy and absolute devotion. Celebrating half a year of holding hands, making jokes, and sharing dreams."
        },
        {
          title: "Month 7: Comfort & Peace",
          date: "February 15, 2026",
          description: "Through the cold winters, your warm love kept my soul cozy. You've officially become my favorite escape and my home."
        },
        {
          title: "Month 8: Growing Together",
          date: "March 15, 2026",
          description: "Stepping into a new year together. Supporting each other's career, goals, and personal dreams. We are the ultimate dream team."
        },
        {
          title: "Month 9: Unconditional Trust",
          date: "April 15, 2026",
          description: "Nine months of seeing my future in your beautiful eyes. A bond built on trust, respect, and deep emotional alignment."
        },
        {
          title: "Month 10: Our Milestone",
          date: "May 15, 2026",
          description: "Happy 10th Month Anniversary, my beautiful fiancée! We've built so many memories, and yet, this is only the beginning."
        }
      ]
    },
    ta: {
      title: "நமது காதல் பயணத்தின் காலவரிசை",
      subtitle: "நாம் ஒன்றாகக் கழித்த 10 அழகான மாதங்களின் மாதவாரியான தொகுப்பு",
      months: [
        {
          title: "மாதம் 1: காதல் பொறி",
          date: "ஆகஸ்ட் 15, 2025",
          description: "எல்லாம் தொடங்கிய இடம். நமது பாதைகள் ஒன்றிணைந்து, நமது வாழ்க்கையில் ஒரு புதிய அழகான அத்தியாயம் தொடங்கிய தருணம். ஒவ்வொரு உரையாடலும் ஒரு புதிய உணர்வைத் தந்தது."
        },
        {
          title: "மாதம் 2: நெருக்கம் வளர்தல்",
          date: "செப்டம்பர் 15, 2025",
          description: "நள்ளிரவுப் பேச்சுகள், முடிவில்லா சிரிப்புகள் மற்றும் இரகசியங்களைப் பகிர்ந்து கொள்ளுதல். நமது பிணைப்பு எவ்வளவு விசேஷமானது மற்றும் தனித்துவமானது என்பதை உணர்ந்த காலம்."
        },
        {
          title: "மாதம் 3: அழகான சந்திப்புகள்",
          date: "அக்டோபர் 15, 2025",
          description: "இனிமையான சந்திப்புகள் மற்றும் அமைதியான தருணங்கள். உன்னுடன் கழித்த ஒவ்வொரு நொடியும் நீ என் வாழ்க்கையில் என்றென்றும் இருக்க வேண்டும் என்பதை உணர வைத்தது."
        },
        {
          title: "மாதம் 4: ஆழமான இணைப்பு",
          date: "நவம்பர் 15, 2025",
          description: "வாழ்க்கையின் சிறிய தடைகளை ஒன்றாக எதிர்கொள்வதும், ஒருவருக்கொருவர் நங்கூரமாக இருப்பதும். உனது முன்னிலையே எனது மிகப்பெரிய ஆறுதலாக மாறியது."
        },
        {
          title: "மாதம் 5: பாதி தூரம்",
          date: "டிசம்பர் 15, 2025",
          description: "ஐந்து மாத அழகான சந்திப்புகளும் இனிமையான பேச்சுகளும். உன் மீதான என் காதல் நாளுக்கு நாள் ஆழமாகவும், வலுவாகவும், உறுதியாகவும் வளர்ந்து வருகிறது."
        },
        {
          title: "மாதம் 6: அரை வருட மகிழ்ச்சி",
          date: "ஜனவரி 15, 2026",
          description: "ஆறு மாத கால தூய மகிழ்ச்சி மற்றும் அர்ப்பணிப்பு. கைகோர்த்து, வேடிக்கைகள் செய்து, கனவுகளைப் பகிர்ந்து கொண்ட அரை வருடத்தைக் கொண்டாடுகிறோம்."
        },
        {
          title: "மாதம் 7: ஆறுதலும் அமைதியும்",
          date: "பிப்ரவரி 15, 2026",
          description: "குளிர்காலத்திலும், உனது கதகதப்பான காதல் என் ஆத்மாவை இதமாக வைத்திருந்தது. நீ அதிகாரப்பூர்வமாக எனக்குப் பிடித்த புகலிடமாகவும் என் இல்லமாகவும் மாறிவிட்டாய்."
        },
        {
          title: "மாதம் 8: இணைந்து வளர்தல்",
          date: "மார்ச் 15, 2026",
          description: "புதிய ஆண்டில் ஒன்றாக அடியெடுத்து வைப்பது. ஒருவருக்கொருவர் தொழில், இலக்குகள் மற்றும் தனிப்பட்ட கனவுகளுக்கு ஆதரவளிப்பது. நாம் ஒரு சிறந்த இணை."
        },
        {
          title: "மாதம் 9: நிபந்தனையற்ற நம்பிக்கை",
          date: "ஏப்ரல் 15, 2026",
          description: "உன் அழகான கண்களில் எனது எதிர்காலத்தைக் கண்ட ஒன்பது மாதங்கள். நம்பிக்கை, மரியாதை மற்றும் ஆழமான உணர்வு ரீதியான பிணைப்பால் உருவான உறவு."
        },
        {
          title: "மாதம் 10: நமது மைல்கல்",
          date: "மே 15, 2026",
          description: "மகிழ்ச்சியான 10வது மாத நிறைவு வாழ்த்துகள், என் அழகான மனைவியே! நாம் பல நினைவுகளை உருவாக்கியுள்ளோம், ஆனால் இது ஒரு ஆரம்பம் மட்டுமே."
        }
      ]
    }
  },
  gallery: {
    en: {
      title: "Photo Memory Gallery",
      subtitle: "A mosaic of our favorite smiles, laughs, and adventures together",
      reactBurst: "Love it!"
    },
    ta: {
      title: "புகைப்பட நினைவுகளின் தொகுப்பு",
      subtitle: "நாம் ஒன்றாகக் கழித்த அழகான புன்னகைகள், சிரிப்புகள் மற்றும் சாகசங்களின் தொகுப்பு",
      reactBurst: "அன்பு!"
    }
  },
  videoMemories: {
    en: {
      title: "Video Memories",
      subtitle: "Moving moments captured in time. Click to play each sweet memory.",
      play: "Play Video"
    },
    ta: {
      title: "வீடியோ நினைவுகள்",
      subtitle: "காலத்தில் பதிவுசெய்யப்பட்ட நகரும் தருணங்கள். ஒவ்வொரு நினைவையும் காண கிளிக் செய்யவும்.",
      play: "வீடியோவை இயக்கு"
    }
  },
  loveInNumbers: {
    en: {
      title: "Our Love in Numbers",
      subtitle: "Quantifying the beautiful parameters of our relationship",
      months: "Months Together",
      days: "Total Days Together",
      photos: "Beautiful Photos",
      videos: "Precious Videos"
    },
    ta: {
      title: "எண்களில் நமது காதல்",
      subtitle: "நமது உறவின் அழகான அளவீடுகளை எண்களில் காண்போம்",
      months: "ஒன்றாக இருந்த மாதங்கள்",
      days: "மொத்த நாட்கள் ஒன்றாக",
      photos: "அழகான புகைப்படங்கள்",
      videos: "விலைமதிப்பற்ற வீடியோக்கள்"
    }
  },
  reasons: {
    en: {
      title: "Reasons I Love You",
      subtitle: "A small list of the infinite things that make me fall for you daily",
      items: [
        { title: "Your Smile", desc: "It brightens my darkest days and instantly fills my heart with absolute peace and warmth." },
        { title: "Your Kindness", desc: "The gentle, empathetic way you care for me and everyone around you melts my heart." },
        { title: "Your Support", desc: "Always standing by me, believing in my dreams even when I struggle to believe in myself." },
        { title: "Your Love", desc: "Pure, unconditional, and the single most beautiful gift I have ever received in my life." },
        { title: "Your Caring Nature", desc: "How you remember every tiny detail about me and look after my happiness with so much care." },
        { title: "Our Memories", desc: "Every laugh we shared, every road we traveled, forever etched in my soul as treasure." }
      ]
    },
    ta: {
      title: "நான் உன்னை காதலிப்பதற்கான காரணங்கள்",
      subtitle: "ஒவ்வொரு நாளும் உன்னை நான் நேசிப்பதற்கான எல்லையற்ற காரணங்களில் சில",
      items: [
        { title: "உன் புன்னகை", desc: "அது என் இருண்ட நாட்களை ஒளிரச் செய்து என் இதயத்தை முழு அமைதியாலும் கதகதப்பாலும் நிரப்புகிறது." },
        { title: "உன் அன்பு", desc: "என் மீதும் உன்னைச் சுற்றியுள்ளவர்கள் மீதும் நீ காட்டும் அன்பான, பரிவுமிக்க அக்கறை என் இதயத்தைக் கரைக்கிறது." },
        { title: "உன் ஆதரவு", desc: "நான் என்னை சந்தேகப்படும்போதும் என் கனவுகளை நம்பி, எப்போதும் என் பின்னே உறுதியாக நிற்பது." },
        { title: "உன் காதல்", desc: "உண்மையான, நிபந்தனையற்ற, என் வாழ்வில் நான் பெற்ற மிக அழகான ஒரே பரிசு." },
        { title: "உன் அக்கறை", desc: "என்னைப்பற்றிய ஒவ்வொரு சிறிய விஷயத்தையும் நினைவில் வைத்து என் மகிழ்ச்சியைக் கவனிப்பது." },
        { title: "நமது நினைவுகள்", desc: "நாம் பகிர்ந்து கொண்ட ஒவ்வொரு சிரிப்பும், பயணித்த ஒவ்வொரு பாதையும் என் ஆத்மாவில் என்றென்றும் பதிந்துள்ளது." }
      ]
    }
  },
  loveLetter: {
    en: {
      title: "A Letter from My Heart",
      subtitle: "Words are not enough, but here is a piece of my soul for you",
      letter: `Dearest,

These past 10 months have been the most magical months of my life. From the moment you entered my world on August 15, 2025, you have filled every day with joy, laughter, and a sense of completeness that I never knew existed.

Thank you for being my anchor, my best friend, and my greatest love. You have stood by me, supported my dreams, and loved me unconditionally. Every smile you share, every laugh we hold, and every quiet moment we spend together is a treasure I will hold close to my heart forever.

As we celebrate this milestone, I want you to know that my love for you has only grown stronger. I promise to cherish you, protect you, and stand by you through every high and low that life brings.

Yours forever and always,
Karthigeyan`
    },
    ta: {
      title: "என் இதயத்திலிருந்து ஒரு கடிதம்",
      subtitle: "வார்த்தைகள் போதாது, ஆனால் உனக்காக என் ஆன்மாவின் ஒரு பகுதி இங்கே",
      letter: `என் அன்பானவளே,

இந்த கடந்த 10 மாதங்கள் என் வாழ்வின் மிகவும் மாயாஜாலமான காலம். ஆகஸ்ட் 15, 2025 அன்று நீ என் உலகிற்குள் நுழைந்த நொடியிலிருந்து, இதுவரை நான் அறியாத மகிழ்ச்சியாலும், சிரிப்பாலும், முழுமை உணர்வாலும் ஒவ்வொரு நாளையும் நிரப்பியுள்ளாய்.

என் நங்கூரமாக, என் சிறந்த நண்பராக, என் மிகப்பெரிய காதலாக இருப்பதற்கு நன்றி. நீ எனக்கு துணையாக நின்று, என் கனவுகளுக்கு ஆதரவளித்து, நிபந்தனையின்றி என்னை நேசித்தாய். நீ காட்டும் ஒவ்வொரு புன்னகையும், நாம் சிரிக்கும் ஒவ்வொரு சிரிப்பும், நாம் ஒன்றாகக் கழிக்கும் ஒவ்வொரு அமைதியான நொடியும் என் இதயத்திற்கு மிக நெருக்கமான பொக்கிஷங்கள்.

இந்த மைல்கல்லை நாம் கொண்டாடும் வேளையில், உன் மீதான என் காதல் இன்னும் வலுவடைந்துள்ளது என்பதை நீ அறிய விரும்புகிறேன். வாழ்க்கை கொண்டு வரும் ஒவ்வொரு இன்ப துன்பங்களிலும் உன்னைப் போற்றிப் பாதுகாக்கவும், உனக்குத் துணையாக நிற்கவும் நான் உறுதியளிக்கிறேன்.

என்றென்றும் உன் அன்புடன்,
கார்த்திகேயன்`
    }
  },
  liveCounter: {
    en: {
      title: "We Have Been Together For",
      subtitle: "Counting every single beautiful second of our togetherness",
      months: "Months",
      days: "Days",
      hours: "Hours",
      minutes: "Minutes",
      seconds: "Seconds"
    },
    ta: {
      title: "நாம் ஒன்றாக இருந்த காலம்",
      subtitle: "நமது இணைவின் ஒவ்வொரு அழகான நொடியையும் எண்ணுகிறோம்",
      months: "மாதங்கள்",
      days: "நாட்கள்",
      hours: "மணிநேரம்",
      minutes: "நிமிடங்கள்",
      seconds: "விநாடிகள்"
    }
  },
  polaroids: {
    en: {
      title: "Memory Polaroids",
      subtitle: "Moments frozen in time, hanging on the strings of our love",
      captions: [
        "The sparkle in your eyes ✨",
        "Our favorite sweet escape 🍰",
        "Holding hands, holding hearts 🤝",
        "Your laugh is my favorite song 🎵",
        "Under the beautiful sky 🌅",
        "In your warm embrace 🤗"
      ]
    },
    ta: {
      title: "நினைவுப் படங்கள்",
      subtitle: "நமது காதலின் நூல்களில் தொங்கும், காலத்தில் உறைந்த தருணங்கள்",
      captions: [
        "உன் கண்களின் ஒளி ✨",
        "நமக்கு பிடித்த இனிமையான தருணம் 🍰",
        "கைகளைக் கோர்த்து, இதயம் தழுவி 🤝",
        "உன் சிரிப்பு எனக்கு பிடித்த இசை 🎵",
        "அழகான வானத்தின் கீழ் 🌅",
        "உன் கதகதப்பான தழுவலில் 🤗"
      ]
    }
  },
  loveNotes: {
    en: {
      title: "Little Love Notes",
      subtitle: "Sticky notes of love scattered across my heart",
      notes: [
        "Thank you for being my happiness. ❤️",
        "You make every day beautiful. 🌸",
        "My favorite place is beside you. 🏡",
        "You are my absolute dream come true. ✨",
        "I love you more than words can say. 💌",
        "You make my soul sing. 🎵"
      ]
    },
    ta: {
      title: "சின்னஞ்சிறு காதல் குறிப்புகள்",
      subtitle: "என் இதயமெங்கும் சிதறிக்கிடக்கும் காதல் குறிப்புகள்",
      notes: [
        "என் மகிழ்ச்சிக்கான காரணம் நீ. ❤️",
        "ஒவ்வொரு நாளையும் அழகாக்குபவள் நீ. 🌸",
        "உன் அருகில் இருப்பதே எனக்கு பிடித்த இடம். 🏡",
        "நீ என் நிஜமான கனவு. ✨",
        "வார்த்தைகளால் சொல்ல முடியாத அளவு உன்னை நேசிக்கிறேன். 💌",
        "நீ என் ஆன்மாவை பாட வைக்கிறாய். 🎵"
      ]
    }
  },
  openWhen: {
    en: {
      title: "Open When Letters",
      subtitle: "A digital box of letters for when you need me most",
      letters: [
        {
          title: "Open When You Miss Me",
          content: "Close your eyes and take a deep breath. Remember that distance is only physical. My heart is beating right next to yours. I am only a phone call or a text away, and I am thinking about you at this very moment. I love you!"
        },
        {
          title: "Open When You Feel Sad",
          content: "I wish I could hold you in my arms and wipe away every tear. Remember how strong you are, and how much happiness you bring into my life. Read through our messages or look at our photos. This sadness is temporary, but my love for you is permanent."
        },
        {
          title: "Open When You Need Motivation",
          content: "You are the smartest, most capable, and most beautiful person I know. There is nothing you cannot achieve. I believe in you with all my heart, and I am always here to cheer you on. Go shine like the star you are!"
        },
        {
          title: "Open When You Want To Smile",
          content: "Remember that goofy face I make? Or the time we got lost but ended up having the best time? You have the most beautiful smile in the world, and my life's mission is to keep it on your face. Smile, my love, you are deeply adored!"
        }
      ]
    },
    ta: {
      title: "எப்போது திறக்க வேண்டும்?",
      subtitle: "உனக்கு என் உதவி தேவைப்படும்போது திறப்பதற்கான காதல் கடிதங்கள்",
      letters: [
        {
          title: "என்னை நினைத்து ஏங்கும்போது",
          content: "உன் கண்களை மூடி ஆழமாக மூச்சு விடு. தூரம் என்பது வெறும் உடல் ரீதியானது மட்டுமே என்பதை நினைவில் கொள். என் இதயம் உன்னுடைய இதயத்தின் அருகிலேயே துடிக்கிறது. நான் ஒரு தொலைபேசி அழைப்பு அல்லது ஒரு குறுஞ்செய்தி தூரத்தில் மட்டுமே இருக்கிறேன், இந்த நொடியிலும் உன்னை நினைத்துக் கொண்டிருக்கிறேன். ஐ லவ் யூ!"
        },
        {
          title: "நீ சோகமாக இருக்கும்போது",
          content: "நான் உன்னை என் கைகளில் அணைத்து, உன் ஒவ்வொரு கண்ணீரையும் துடைக்க விரும்புகிறேன். நீ எவ்வளவு வலிமையானவள், என் வாழ்க்கையில் நீ எவ்வளவு மகிழ்ச்சியைக் கொண்டு வருகிறாய் என்பதை நினைவில் கொள். நமது செய்திகளைப் படி அல்லது புகைப்படங்களைப் பார். இந்த சோகம் தற்காலிகமானது, ஆனால் உன் மீதான என் காதல் நிரந்தரமானது."
        },
        {
          title: "உனக்கு ஊக்கம் தேவைப்படும்போது",
          content: "நீ நான் அறிந்த மிக புத்திசாலி, திறமையான மற்றும் அழகான நபர். உன்னால் சாதிக்க முடியாதது எதுவுமில்லை. நான் என் முழு இதயத்தோடு உன்னை நம்புகிறேன், உன்னை ஊக்குவிக்க எப்போதும் இருக்கிறேன். நீ ஒரு நட்சத்திரத்தைப் போல பிரகாசிப்பாய்!"
        },
        {
          title: "நீ புன்னகைக்க விரும்பும்போது",
          content: "நான் செய்யும் வேடிக்கையான முகபாவனைகள் நினைவிருக்கிறதா? அல்லது நாம் தொலைந்து போய், பின் மிகச் சிறந்த தருணத்தை அடைந்தோமே? உலகில் மிக அழகான புன்னகை உன்னுடையதுதான், அதை உன்னிடமே வைத்திருக்க வைப்பதே என் வாழ்வின் நோக்கம். புன்னகை செய் என் அன்பே!"
        }
      ]
    }
  },
  secretSurprise: {
    en: {
      button: "Click For A Special Surprise ❤️",
      message: "You are my dream come true! I am the luckiest person in the universe to have you. Happy 10th Month Anniversary! 💖🎉"
    },
    ta: {
      button: "ஒரு சிறப்பு ஆச்சரியத்திற்கு இங்கே கிளிக் செய்யுங்கள் ❤️",
      message: "நீ என் நிஜமான கனவு! உன்னை அடைய நான் இந்த பிரபஞ்சத்திலேயே அதிர்ஷ்டசாலி. 10வது மாத ஆண்டு நிறைவு வாழ்த்துகள்! 💖🎉"
    }
  },
  grandFinale: {
    en: {
      text: "These 10 months are only the beginning of our forever story ❤️"
    },
    ta: {
      text: "இந்த 10 மாதங்கள் நமது நிரந்தர காதல் பயணத்தின் தொடக்கம் மட்டுமே ❤️"
    }
  },
  celebration: {
    en: {
      title: "Happy 10th Month Anniversary My Love ❤️"
    },
    ta: {
      title: "10வது மாத ஆண்டு நினைவு வாழ்த்துகள் என் அன்பே ❤️"
    }
  }
};
